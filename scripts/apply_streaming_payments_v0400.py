...
def replace_once(text: str, old: str, new: str, label: str) -> str:
    if old not in text:
        raise SystemExit(f"No se encontró {label}: {old}")
    return text.replace(old, new, 1)

p = Path("panel/streaming.js")
s = p.read_text(encoding="utf-8")
if "Control de cobros v0.4.0" not in s:
    s = replace_once(s, "núcleo operativo v0.3.1", "núcleo operativo v0.4.0", "versión del núcleo")
    s = replace_once(s, "/panel/streaming.css?v=0300", "/panel/streaming.css?v=0400", "cache CSS")
    s += r'''
...
s = p.read_text(encoding="utf-8")
s = s.replace("Streaming · Versión v0.3.1", "Streaming · Versión v0.4.0")
s = s.replace("/panel/streaming.js?v=0301", "/panel/streaming.js?v=0400")
p.write_text(s, encoding="utf-8")

p = Path("panel/demo.html")
s = p.read_text(encoding="utf-8").replace("Versión demo · v0.3.1", "Versión demo · v0.4.0")
p.write_text(s, encoding="utf-8")

p = Path("tests/streaming-check.mjs")
s = p.read_text(encoding="utf-8")
for old, new in [
    ("Streaming · Versión v0.3.1", "Streaming · Versión v0.4.0"),
    ("/panel/streaming.js?v=0301", "/panel/streaming.js?v=0400"),
    ("núcleo operativo v0.3.1", "núcleo operativo v0.4.0"),
    ("Versión demo · v0.3.1", "Versión demo · v0.4.0"),
    ("Panel Streaming v0.3.1", "Panel Streaming v0.4.0"),
]:
    s = s.replace(old, new)
needle = "  'tu renovación de'\n"
if "Control de cobros v0.4.0" not in s:
    if needle not in s:
        raise SystemExit("No se encontró marcador de pruebas")
    s = s.replace(needle, "  'tu renovación de',\n  'Control de cobros v0.4.0',\n  'streamingOpenPayment',\n  'payment_status',\n  'Pendientes de cobro'\n", 1)
p.write_text(s, encoding="utf-8")

for name in ["package.json", "package-lock.json"]:
    p = Path(name)
    s = p.read_text(encoding="utf-8").replace('"version": "0.3.1"', '"version": "0.4.0"')
    p.write_text(s, encoding="utf-8")

print("Parche Streaming v0.4.0 aplicado")
